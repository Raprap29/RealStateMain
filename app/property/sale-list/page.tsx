...
const salelist = () =>{
  ...
  useEffect(()=>{
    ...
    return(
        <>
        <title>Jama Realty - For Sale</title>
        <ContentSLide />
        <div className="mx-auto container max-w-[1100px] mt-5">
          <div className="text-center">
            <p className="font-bold text-[35px]">FOR SALE</p>
            <p className="mt-4">Explore our vast collection of properties for sale, featuring an array of options including vacant lots, preselling condominium units, and fully-equipped commercial spaces – offering excellent opportunities for both investment and personal use.</p>
          </div>
            <div className="px-[10px] grid grid-cols-3 gap-4 mt-8 max-[740px]:grid-cols-3 max-[560px]:grid-cols-2 max-[460px]:grid-cols-1">
                {assetsItemImg.map((item, index) => (
                    <div key={index} className="flex justify-center relative">
                        <Link href={`/property/sale?type=${item.Code}`}>
